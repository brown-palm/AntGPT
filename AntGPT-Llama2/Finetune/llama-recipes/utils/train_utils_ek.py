    ...
    for epoch in range(train_config.num_epochs):
        ...
        with MemoryTrace() as memtrace:  # track the memory usage
            ...
            for step, batch in enumerate(tqdm(train_dataloader,colour="blue", desc=f"Training Epoch{epoch}")):
                ...
                if train_config.decoder == 'head':
                    train_mAP_preds.extend(outputs.logits.sigmoid())
                    labels.extend(torch.tensor(batch['labels']))
                ...
                if train_config.enable_fsdp:
                    if rank==0:       
                        print(f"\n step {step} is completed and loss is {step_loss}")
                else:
                    print(f"\n step {step} is completed and loss is {step_loss}")
        ...
        if train_config.decoder == 'head':
            train_mAP_preds = torch.stack(train_mAP_preds)
            labels = torch.stack(labels)
            mAP = mean_AP(train_mAP_preds[:,eval_dataloader.dataset.label_mask[0]],labels[:,eval_dataloader.dataset.label_mask[0]])
            print(f" training {mAP=}")
        ...
        if train_config.run_validation:
            ...
            if train_config.save_model and eval_epoch_loss < best_val_loss:
                ...
                if train_config.use_peft:
                    ...
                    torch.save(model.score.state_dict(), os.path.join(train_config.output_dir, str(epoch), 'head_parameters.pth'))
                    if train_config.enable_fsdp:
                        if rank==0: 
                            print(f"PEFT modules are saved in {train_config.output_dir} directory")
                    else:
                        print(f"PEFT modules are saved in {train_config.output_dir} directory")
                elif not train_config.use_peft and fsdp_config is None:
                    if not os.path.exists(train_config.output_dir):
                        os.makedirs(train_config.output_dir)
                    torch.save(model.score.state_dict(), os.path.join(train_config.output_dir,'head_parameters.pth'))        
                else:
...
def evaluation(model,train_config, eval_dataloader, local_rank, tokenizer):
    ...
    with MemoryTrace() as memtrace:
        for step, batch in enumerate(tqdm(eval_dataloader,colour="green", desc="evaluating Epoch")):
            ...
            if train_config.decoder == 'head':
                mAP_preds = outputs.logits.sigmoid()
                eval_mAP_preds.extend(mAP_preds)
                labels.extend(torch.tensor(batch['labels']))
    ...
    if train_config.decoder == 'head':
        eval_mAP_preds = torch.stack(eval_mAP_preds)
        labels = torch.stack(labels)
        mAP = mean_AP(eval_mAP_preds[:,eval_dataloader.dataset.label_mask[0]],labels[:,eval_dataloader.dataset.label_mask[0]])
